...
func newTestMender(_ *stest.TestOSCalls, config conf.MenderConfig,
	...
	if pieces.AuthMgr == nil {
		...
		ks := store.NewKeystore(pieces.Store, conf.DefaultKeyFile, defaultKeyPassphrase)
...
func Test_Bootstrap(t *testing.T) {
	...
	k := store.NewKeystore(mam.store, conf.DefaultKeyFile, defaultKeyPassphrase)
	assert.NotNil(t, k)
...
func Test_BootstrappedHaveKeys(t *testing.T) {
	...
	// generate valid keys
	ms := store.NewMemStore()
	k := store.NewKeystore(ms, conf.DefaultKeyFile, defaultKeyPassphrase)
	assert.NotNil(t, k)
